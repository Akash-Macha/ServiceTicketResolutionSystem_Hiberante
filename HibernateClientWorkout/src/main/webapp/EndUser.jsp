...
*/
		List<Object[]> listOfDepartments = (List<Object[]>) session.getAttribute("listOfDepartments");
		for(Object eachObject : listOfDepartments) {
			ArrayList eachDepartment = (ArrayList)eachObject;
%>
  			<option value="<%= eachDepartment.get(0) %>"><%= eachDepartment.get(1) %></option>
<%
		}
%>
</select><br>
...
%>
			<option value="<%= eachPriority.get(0) %>"><%= eachPriority.get(1) %></option>
<%
		}
%>
</select><br>
